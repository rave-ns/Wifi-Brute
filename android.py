...
class CrackingTask(BoxLayout):
    ...
    def __init__(self, network, **kwargs):
        ...
        ssid_label = Label(text=self.ssid, halign='left', valign='middle', 
                          size_hint_x=0.7, color=get_color_from_hex(Colors.CYAN))
        ssid_label.bind(size=ssid_label.setter('text_size'))
        header.add_widget(ssid_label)
        
        self.status_label = Label(text=self.status, halign='right', valign='middle', 
                                 size_hint_x=0.3, color=get_color_from_hex(Colors.YELLOW))
        self.status_label.bind(size=self.status_label.setter('text_size'))
        header.add_widget(self.status_label)
        self.add_widget(header)
        
        self.password_label = Label(text="", halign='left', valign='middle', 
...
class WiFiCrackApp(App):
    ...
    def build(self):
        ...
        title = Label(text='WiFi Crack', color=get_color_from_hex(Colors.CYAN), 
                     font_size='24sp', size_hint_x=0.7)
        ...
        log_box.add_widget(Label(text='Log Output', size_hint_y=None, height=30, 
                                halign='left', color=get_color_from_hex(Colors.BLUE)))
        
        self.log_scroll = ScrollView(bar_width=10)
        self.log_output = Label(text='', halign='left', valign='top', 
        ...
        networks_header.add_widget(Label(text='Available Networks', halign='left', 
                                        color=get_color_from_hex(Colors.BLUE), size_hint_x=0.7))
        
        self.scan_button = Button(text='Scan', size_hint_x=0.3, 
                                 background_color=get_color_from_hex(Colors.BLUE))
        ...
        self.crack_button = Button(text='Start Cracking', disabled=True, 
                                  background_color=get_color_from_hex(Colors.GREEN))
        ...
        self.stop_button = Button(text='Stop', disabled=True, 
                                 background_color=get_color_from_hex(Colors.RED))
    ...
    def crack_network_thread(self, network):
        progress_callback = lambda current, total, pwd: Clock.schedule_once(
            lambda dt: self.update_cracking_progress(network, current, total, pwd), 0)
